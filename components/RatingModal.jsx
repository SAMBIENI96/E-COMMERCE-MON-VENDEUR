...
const RatingModal = ({ setRatingModal }) => {
    ...
    const handleSubmit = async () => {
        if (rating < 0 || rating > 5) {
            return toast('Veuillez choisir une note');
        }
        if (review.length < 5) {
            return toast('Ajoutez un petit commentaire');
        }
    ...
    return (
        <div className='fixed inset-0 z-120 flex items-center justify-center bg-black/10'>
            <div className='bg-white p-8 rounded-lg shadow-lg w-96 relative'>
                ...
                <h2 className='text-xl font-medium text-slate-600 mb-4'>Noter le produit</h2>
                ...
                <textarea
                    ...
                    placeholder='Dites-nous ce que vous avez pense du produit'
                    rows='4'
                    value={review}
                    onChange={(e) => setReview(e.target.value)}
                ></textarea>
                <button onClick={e => toast.promise(handleSubmit(), { loading: 'Envoi en cours...' })} className='w-full bg-green-500 text-white py-2 rounded-md hover:bg-green-600 transition'>
                    Envoyer mon avis
                </button>
            </div>
        </div>
