    ...
    return (
        <div className="my-18 text-sm text-slate-600">
            ...
            <div className="flex border-b border-slate-200 mb-6 max-w-2xl">
                {['Description', 'Avis'].map((tab, index) => (
                            ...
                            <div>
                                <div className="flex items-center" >
                                    {Array(5).fill('').map((_, ratingIndex) => (
                                        <StarIcon key={ratingIndex} size={18} className='text-transparent mt-0.5' fill={item.rating >= ratingIndex + 1 ? "#00C950" : "#D1D5DB"} />
                                    ))}
                                </div>
            ...
            <div className="flex gap-3 mt-14">
                ...
                <div>
                    <p className="font-medium text-slate-600">Vendu par {product.store.name}</p>
                    <Link href={`/shop/${product.store.username}`} className="flex items-center gap-1.5 text-green-500">Voir la boutique <ArrowRight size={14} /></Link>
                </div>
            </div>
        </div>
    )
